    ...
    def __init__(self, *, fmt, settings: LoggerSettings, datefmt=None):
        ...
        if settings.time_zone:
            try:
                ...
                logging.info(f"Fuso horário dos logs de arquivos definido como: {self.timezone}")
            except pytz.UnknownTimeZoneError:
                logging.error(
                    f"Logs de arquivos: fuso horário inválido: {settings.time_zone}")
    ...
    def __init__(self, *, fmt, settings: LoggerSettings, datefmt=None):
        ...
        if settings.time_zone:
            try:
                self.timezone = pytz.timezone(settings.time_zone)
                logging.info(
                    f"Fuso horário dos logs do console definido como: {self.timezone}")
            except pytz.UnknownTimeZoneError:
                logging.error(
                    f"Logs do console: fuso horário inválido: {settings.time_zone}")
        logging.Formatter.__init__(self, fmt=fmt, datefmt=datefmt)
