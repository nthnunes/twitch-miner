...
class TwitchChannelPointsMiner:
    ...
    def __init__(
        ...
        logger.info(
            f"Twitch Miner v1.9.9.1 (feito com ❤️ por o tal do nunes)"
        )
    ...
    def run(
        ...
        if self.running:
            ...
            logger.info(
                f"Seção iniciada: '{self.session_id}'", extra={"emoji": ":bomb:"}
            )
            self.running = True
            ...
            logger.info(
                f"Carregando dados para {len(streamers_name)} streamers. Aguarde...",
